...
#' Record an already-computed status from a rendered check table.
#'
#' The course-schedule pipeline renders its checks through check_gt(), which
#' shows a PASS / CHECK / FAIL badge on a gt table. Those badges are readable
#' one at a time but there is no way to see, at the end of a long render,
#' whether anything failed. This lets check_gt() report into the same log the
#' rest of the pipeline uses, without changing how a single check is written.
#'
#' @param status "PASS", "CHECK" or "FAIL" (case-insensitive), or NULL.
chk_from_badge <- function(check, status, value = NULL, detail = NULL) {
  if (is.null(status) || !nzchar(status)) {
    .chk_record(check, "info", "INFO", value, detail)
    return(invisible(NULL))
  }
  st <- toupper(status)
  result <- switch(st, PASS = "PASS", CHECK = "REVIEW", FAIL = "FAIL", "INFO")
  sev    <- if (result == "PASS" || result == "FAIL") "badge" else "badge"
  .chk_record(check, sev, result, value, detail)
  invisible(result)
}
...
chk_report <- function(print_table = TRUE) {
  ...
  n_badge <- sum(log$severity == "badge")
  n_rev   <- sum(log$result   == "REVIEW")
  ...
  cat("  Structural checks : ", n_stop, " (", n_stop - n_fail, " passed, ",
      n_fail, " failed)\n", sep = "")
  if (n_badge > 0) {
    cat("  Rendered checks   : ", n_badge, " (",
        sum(log$result == "PASS" & log$severity == "badge"), " PASS, ",
        n_rev, " CHECK, ",
        sum(log$result == "FAIL" & log$severity == "badge"), " FAIL)\n", sep = "")
  }
