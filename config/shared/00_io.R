...
# DISTRICT-AGNOSTIC.
#
# A staged pipeline that writes an intermediate file and reads it back on the
# next stage pays for two SharePoint round trips and gains a file nobody asked
# for. This does one read per source file, caches it as .rds, and leaves the
# writing to whatever actually needs to leave the pipeline.
#
...
#' @param clean_names Apply janitor::clean_names(). Default FALSE, because a
